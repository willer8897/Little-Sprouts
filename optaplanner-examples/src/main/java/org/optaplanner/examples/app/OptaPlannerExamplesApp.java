...
public class OptaPlannerExamplesApp extends JFrame {
    ...
    private JPanel createDifficultExamplesPanel() {
        ...
        //TODO: remove this junky test code
        
        /*
        try {
            
        	System.out.println("Doing the stuff");
            File fXmlFile = new File("C://solution.xml");
        	...
        	for (int temp = 0; temp < nList.getLength(); temp++) {
        		...
        		if (nNode.getNodeType() == Node.ELEMENT_NODE) {
        			...
        			map.put(eElement.getAttribute("id"), eElement.getElementsByTagName("code").item(0).getTextContent());
        	...
        	NodeList nList3 = doc.getElementsByTagName("Shift");
        	System.out.println("there are " + nList3.getLength() + " shift tags");
        	for (int temp = 0; temp < 200; temp++) {
        		...
        		if (nNode.getNodeType() == Node.ELEMENT_NODE) {
        			...
        			Element eElement = (Element) nNode;
        			Element shiftType = (Element) eElement.getElementsByTagName("shiftType").item(0);
        			
        			
        			if(shiftType.getElementsByTagName("code").getLength() != 0 && Integer.parseInt(eElement.getAttribute("id")) < 1000)
        			{
        				shiftMap.put(eElement.getAttribute("id"), shiftType.getElementsByTagName("code").item(0).getTextContent());
        				System.out.println("Shift reference " + eElement.getAttribute("id") + " is actually " + shiftType.getElementsByTagName("code").item(0).getTextContent());
        	...
        	NodeList nList4 = doc.getElementsByTagName("shiftList");
        	//System.out.println("The size of nList4 is " + nList4.getLength());
        	for (int temp = 0; temp < nList4.getLength(); temp++) {
        		...
        		//System.out.println("\nCurrent Element :" + nNode.getNodeName());
        		//System.out.println("Mapping shift??");
        		...
        		if (nNode.getNodeType() == Node.ELEMENT_NODE) {
        			...
        			Element eElement = (Element) nNode;
        			//System.out.println("The id is " + eElement.getAttribute("id"));
        			
        			NodeList list = eElement.getElementsByTagName("Shift");
        			for(int i = 0; i < list.getLength(); i++)
        			{
        				//System.out.println("Mapping shift");
        				if(list.item(i).getNodeType() == Node.ELEMENT_NODE)
        				{
        					Element shift = (Element) list.item(i);
        					Element shiftType = (Element) shift.getElementsByTagName("shiftType").item(0);
        					if(shiftType.getAttribute("reference").toString().compareTo("") != 0)
        					{
        						System.out.println("The shift " + shift.getAttribute("id") + " is really" + shiftType.getAttribute("reference"));
        						Integer integer = Integer.parseInt(shiftType.getAttribute("reference"));
        						shiftMap.put(shift.getAttribute("id"), integer - 1 );
        					}
        				}
        			}
        	...
        	NodeList nList2 = doc.getElementsByTagName("ShiftAssignment");
        	for (int temp = 0; temp < nList2.getLength(); temp++) {
        		...
        		if (nNode.getNodeType() == Node.ELEMENT_NODE) {
        			...
        			Element eElement = (Element) nNode;
        			Element eElement2 = (Element) eElement.getElementsByTagName("shift").item(0);
        			System.out.println("Shift reference " + eElement2.getAttribute("reference") + " maps to " + shiftMap.get(eElement2.getAttribute("reference")));
